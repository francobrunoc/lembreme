...
export default function EnviadoEtiquetasPage() {
    const router = useRouter()
    const [codigoPedido, setCodigoPedido] = useState(null)
    let [message, setMessage] = useState(null)

    useEffect(() => {
        setCodigoPedido(store.get('codigoPedido'))
        pedidosService.update(store.get('pedido')).then((res) => {
    ...
    return (
        <DefaultCenteredLayout>
            ...
            <div className="row justify-center body__title--text" style={{marginBottom: '3.15vh'}}>Pedido { codigoPedido } enviado para etiquetas</div>
